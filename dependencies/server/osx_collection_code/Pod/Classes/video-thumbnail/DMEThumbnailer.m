...
- (void)generateImageThumbnails:(NSString *)aPath afterGenerate:(GenerateThumbCompletionBlock)afterBlock completionBlock:(GenerateThumbsCompletionBlock)block {
   ...
   for (NSString * prefix in self.sizes) {
      ...
      [self generateImageThumbnail:aPath widthSize:size widthPrefix:prefix completionBlock:^(NSImage ** thumb) {
          if (afterBlock) {
             afterBlock(thumb);
          }
          if (thumb) {
...
- (void)generateVideoThumbnails:(NSString *)aPath afterGenerate:(GenerateThumbCompletionBlock)afterBlock completionBlock:(GenerateThumbsCompletionBlock)block {
   ...
   for (NSString * prefix in self.sizes) {
      ...
      CGSize objectForKey = [self.sizes objectForKey:prefix];
      CGSize size = objectForKey;
      [[self.sizes objectForKey:prefix] CGSizeValue];
      [self generateVideoThumbnail:aPath
                         widthSize:size
                          atSecond:1
                       widthPrefix:prefix
                   completionBlock:^(NSImage ** thumb) {
                       if (afterBlock) {
                          afterBlock(thumb);
                       }
                       if (thumb) {
...
- (void)generatePDFThumbnails:(NSString *)aPath afterGenerate:(GenerateThumbCompletionBlock)afterBlock completionBlock:(GenerateThumbsCompletionBlock)block {
   ...
   for (NSString * prefix in self.sizes) {
      CGSize size = [[self.sizes objectForKey:prefix] CGSizeValue];
      [self generatePDFThumbnail:aPath widthSize:size forPage:1 widthPrefix:prefix completionBlock:^(NSImage ** thumb) {
          if (afterBlock) {
             afterBlock(thumb);
          }
          if (thumb) {
...
- (void)generateImageThumbnail:(NSString *)aPath widthSize:(CGSize)aSize widthPrefix:(NSString *)aPrefix completionBlock:(GenerateThumbCompletionBlock)block {
   ...
   if ([self thumbExistForPath:aPath andPrefix:aPrefix]) {
      ...
      if (data) {
         ...
         thumbnail = [self imageByScalingAndCropping:[NSImage imageWithData:data] forSize:aSize];
...
- (void)generateVideoThumbnail:(NSString *)aPath widthSize:(CGSize)aSize atSecond:(NSInteger)aSecond widthPrefix:(NSString *)aPrefix completionBlock:(GenerateThumbCompletionBlock)block {
   if ([self thumbExistForPath:aPath andPrefix:aPrefix]) {
      NSImage * thumbnail = [self readThumb:aPath withPrefix:aPrefix];
      if (block) {
         block(&thumbnail);
      }
   }
      ...
      [asset loadValuesAsynchronouslyForKeys:[NSArray arrayWithObject:@"duration"] completionHandler:^{
          ...
          [generator generateCGImagesAsynchronouslyForTimes:[NSArray arrayWithObject:[NSValue valueWithCMTime:thumbTime]]
                                          completionHandler:^(CMTime requestedTime, CGImageRef image, CMTime actualTime, AVAssetImageGeneratorResult result, NSError * error) {
                                              NSImage * thumbnail = [NSImage imageWithCGImage:image];
...
- (NSImage *)readThumb:(NSString *)aPath withPrefix:(NSString *)aPrefix {
   return [NSImage imageWithContentsOfFile:[self thumbPathFromFilePath:aPath andPrefix:aPrefix]];
}
...
- (BOOL)saveThumb:(NSImage *)aImage inPath:(NSString *)aPath withPrefix:(NSString *)aPrefix {
   NSFileManager * filemgr = [NSFileManager defaultManager];

   NSString * urlDirectorio = [NSString stringWithFormat:@"%@/%@",
   ...
   if ([filemgr changeCurrentDirectoryPath:urlDirectorio] == YES) {
      [NSImagePNGRepresentation(aImage) writeToFile:[self thumbPathFromFilePath:aPath andPrefix:aPrefix]
                                         atomically:YES];
      return YES;
   }
   else {
...
- (NSImage *)imageByScalingAndCropping:(NSImage *)aImage forSize:(CGSize)aSize {
   NSImage * sourceImage = aImage;
